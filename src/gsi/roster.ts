...
export class RosterManager {
  ...
  private deriveLastMan(team: TeamContext | undefined): CoachEvent | null {
    ...
    // man. lastManFreshMs (default 3s) is far above a live player's update rate yet
    // well under the 10s heartbeat — tight enough that a stale "alive" can't slip
    // through as a clutch.
    ...
    // that same tight window. A teammate whose "dead" status comes from a stale
    // frame may actually still be up (we just haven't seen their newer alive frame),
    // so asserting last-man off it would be a lie. Never assert last-man unless the
    // whole team's status is freshly confirmed, not just the survivor's.
