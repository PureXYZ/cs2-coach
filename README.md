...
| Your kills this round, K/D/A, MVPs | **Enemy** health, money, weapons (and teammates' — *unless they run the coach too*, see below) |
...
- **Honest by default** — the coach only ever speaks about the players it can actually see. It won't claim "you're the last one alive" or "everyone's broke" unless you tell it the squad is fully wired: set `COACH_SQUAD_SIZE` (e.g. `5`) and those whole-team calls unlock; leave it unset and it always hedges to "the players I can see" — safe even if someone forgets to launch.
