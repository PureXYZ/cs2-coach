...
- ❌ Any pre-2025 tutorial/library (@discordjs/voice < 0.19, sodium deps).

Other verified facts driving the implementation:
- On Node ≥ 22.12 no sodium library is needed (built-in aes-256-gcm).
- **ffmpeg is skippable at unity volume only**: feeding pre-encoded Ogg/Opus or WebM/Opus into `createAudioResource` uses a demux-only path (no ffmpeg, no transcode); raw 48 kHz s16le PCM uses an Opus-encode-only path. The TTS providers were chosen to emit these formats natively, so the *unity-gain* fast path needs no ffmpeg.
- **A non-unity gain DOES need ffmpeg**: any inline volume (`COACH_VOLUME` ≠ 1, or a per-voice volume in `ELEVENLABS_VOICES`) is transcoded by `@discordjs/voice` through **ffmpeg**, not opusscript. For an OggOpus input its transformer graph picks the lowest-cost path `ffmpeg → PCM → volume → Opus`, and `prism-media` throws `FFmpeg/avconv not found!` if the binary is absent rather than falling back to an opus-only decode/encode route. So the image **bundles ffmpeg** (see the Dockerfile), and running locally with a non-unity gain needs ffmpeg on PATH. (`opusscript` is still a dependency `@discordjs/voice` can use for Opus codec work, but it does not substitute for ffmpeg on the inline-volume path — an earlier note here wrongly claimed it did, which shipped an ffmpeg-less image that failed the first time a per-voice volume was set.)
